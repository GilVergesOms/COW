...
<body>
	<?php
		//Dades necessaries per la conexio amb la DB
		$host = "localhost";
		$username = "root";
		$db1 = "simpsons";
		$db2 = "world";
		...
		// Creant la conexio
		$conn = mysqli_connect($host, $username,"", $db1)or die("Connection failed");

		//Llegim certs valors del formolari que inserirem a la taula Student de la base de dades Simpson
		$nombre = $_POST['nombre'];
		$mail = $nombre."@cow.cat";
		$psw = $_POST['apellido'];

		//Agafo els IDs de students per poder comprobar que el nou element que inserim no repeteixi ID
		$con_id = "SELECT id FROM students";
		$id_list =  mysqli_query($conn, $con_id)or die("No puc obtenir els IDs");

		//Recorro l'array de IDs retornat per la consulta per trobar un ID que no estigui a la taula
		$id = 0;
		$exist = 0;
		...
		//Un cop trobat l'ID, insereixo a la taula
		$insertar = "INSERT INTO students (email, id, name, password) VALUES ('$mail', '$id', '$nombre', '$psw')";

		$resultado = mysqli_query($conn, $insertar)or die("Insertion failed");
		...
		<div class="col-xs-12">
			<div class="container">
				...
				<div class="table-responsive">
					<?php
					//Forma part de l'exercici anterior. Envio a través de un POST les dades del formulari i les represento en una taula
					$desti = $_POST['Desti'];
					$adults = $_POST['Adults'];
					$origen = $_POST['Origen'];
					$nens = $_POST['Nens'];
